...
2. **생성된 백업 볼트 확인**: AWS 백업 콘솔에서 `[볼트]` 메뉴로 이동하여 생성된 백업 볼트(`gr-product-bv-ec2`)와 상세 정보를 확인한다.
3. **생성된 백업 계획 확인**: `[백업 계획]` 메뉴로 이동하여 생성된 백업 계획(`gr-product-bp-ec2`)과 백업 규칙을 확인한다. 백업 빈도, 시작 시간, 보존 기간 등의 상세 정보를 볼 수 있다.    
4. **할당된 리소스 확인**: 백업 계획 내 `[리소스 할당]`을 클릭하여 어떤 리소스(`gr-product-selection-ec2`)가 백업 대상인지 확인한다. 이 할당은 특정 태그를 가진 EC2 인스턴스를 대상으로 한다.   
